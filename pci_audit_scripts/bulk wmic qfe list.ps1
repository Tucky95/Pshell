...
### Change csv/txt document with list of servers you need to run on ###
    ...
    ### This script needs to be in the local machines c:\installs to run ###
        ...
        C:\PSTOOLS\PSexec.exe \\$server -h -d powershell.exe -command "& { wmic qfe list | FT caption, csname, description, hotfixid, installdate, installedby, installedon | out-file c:\Installs\PCI_Audit_files\wmic_qfe_list.txt }"
